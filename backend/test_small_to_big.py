...
async def test_small_to_big():
    ...
    test_file_content = (
        "This is paragraph 1. It is short but has some unique words like ABRACADABRA.\n\n"
        "This is paragraph 2. Just filler text to make the document longer.\n\n"
        "This is paragraph 3. More filler text to stretch it out.\n\n"
        "This is paragraph 4. We want a document big enough to test hierarchical chunking.\n\n"
        "This is paragraph 5. The total length should exceed embedding chunk size.\n\n"
    ...
    if results:
        ...
        print("\n--- Retrieval Success ---")
        print(f"Matched Child Chunk ID? (No, replaced with Parent): {top_result.chunk_id}")
        ...
        if len(top_result.content) > settings.DEFAULT_EMBEDDING_CHUNK_SIZE:
            print("=> SUCCESS: The LLM receives the large PARENT context, not the small child chunk!")
        else:
